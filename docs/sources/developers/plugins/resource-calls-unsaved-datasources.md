...
const AWSConfigEditor = ({ options, onOptionsChange }) => {
  ...
  const handleDatabaseDropdownOpen = async () => {
    await saveIfNeeded();
    const data = await datasource.getResource('databases');
    setDatabases(data);
  };
  ...
  return (
    <div>
      ...
      <Select
        options={databases}
        onOpen={handleDatabaseDropdownOpen}
        placeholder={isSaved ? 'Select a database' : 'Save data source first'}
      />
    </div>
  );
};
...
- [Azure Monitor Data Source](https://github.com/grafana/grafana/tree/main/public/app/plugins/datasource/azuremonitor) - Example implementation using a similar pattern
- [CloudWatch Data Source](https://github.com/grafana/grafana/tree/main/public/app/plugins/datasource/cloudwatch) - Example implementation with error handling
